...
def parse_worker(queue_html_links):
    ...
    while True: # постоянно крутим цикл парсинга, пока не получим сигнал остановки
        ...
        if html: # Если пришло что-то осмысленное
            ...
            if parser_func:
                ...
                result = parser_func(html, link_data['base_url'])
...
async def star_download_an_parse():
    ...
    parser_thread = threading.Thread(target=parse_worker, args=(queue_html_links,), daemon=True)
    parser_thread.start()
